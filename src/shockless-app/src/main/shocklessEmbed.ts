...
interface LaunchContext {
  ...
  readonly realm: OriginsRealmDefinition;
...
export class ShocklessEmbedController {
  ...
  constructor(
    ...
    const cacheRoot = options.cacheNamespace
      ? appDataStorePath(options.appDataPath, options.cacheNamespace)
      : appDataStorePath(options.appDataPath);
  ...
  private resolveContext(): LaunchContext | null {
    try {
      ...
      const realm = originsRealmDefinition(shocklessSettings.realm);
...
export function buildShocklessEmbedUrl(baseUrl: string, context: LaunchContext): string {
  const gameConnection = originsRelayGameConnection(context.realm);
  const musConnection = originsRelayMusConnection(gameConnection, context.realm);
  const url = new URL(baseUrl);
  url.searchParams.set("profile", context.profile.id);
  url.searchParams.set("profileVersion", context.profile.versionId);
  url.searchParams.set("realm", context.realm.id);
  ...
  const diagnosticQuery = process.env.SHOCKLESS_ENGINE_QUERY || "";
  if (diagnosticQuery.trim()) {
    const params = new URLSearchParams(diagnosticQuery);
...
export function normalizeOriginsExternalVariables(
  ...
  gameConnection?: { readonly host: string; readonly port: number },
  realmId: OriginsRealmId = DEFAULT_ORIGINS_REALM,
): string {
  const realm = originsRealmDefinition(realmId);
  const resolvedGameConnection = gameConnection ?? originsRelayGameConnection(realm);
  ...
  const musConnection = originsRelayMusConnection(resolvedGameConnection, realm);
  setExternalVariable(lines, keys, "connection.info.host", resolvedGameConnection.host);
  setExternalVariable(lines, keys, "connection.info.port", String(resolvedGameConnection.port));
  ...
  setExternalVariable(lines, keys, "cast.entry.2", realm.patchCast);
  setExternalVariable(lines, keys, "cast.entry.11", realm.nativeEntryView);
  setExternalVariable(lines, keys, "url.prefix", new URL(realm.webOrigin).hostname);
  setExternalVariable(lines, keys, "external.texts.txt", "external_texts.txt");
  setExternalVariable(lines, keys, "external.figurepartlist.txt", "figuredata.xml");
...
class EmbeddedStaticServer {
  ...
  private readonly realmGamedata: OriginsRealmGamedataCache;

  constructor(private readonly cacheRoot: string) {
    this.realmGamedata = new OriginsRealmGamedataCache(cacheRoot);
  }
  ...
  private async handleOriginsData(context: LaunchContext, pathname: string, response: ServerResponse): Promise<void> {
    ...
    if (rootKey === "client") {
      ...
      if (normalizedRequestPath === "external_variables.txt" || normalizedRequestPath === "external_texts.txt") {
        ...
        const profileTexts = readOptionalText(safeJoin(clientRoot, "external_texts.txt"));
        const realmData = await this.realmGamedata.load(context.realm, profileVariables, profileTexts);
        if (normalizedRequestPath === "external_variables.txt") {
          this.log(
            `realm=${context.realm.id} variables=${realmData.variablesSource} texts=${realmData.textsSource}`,
          );
          sendText(
            response,
            200,
            normalizeOriginsExternalVariables(
              realmData.variables,
              context.settings.versionCheckBuild,
              originsRelayGameConnection(context.realm),
              context.realm.id,
            ),
          );
        } else {
          sendText(response, 200, realmData.texts);
        }
        return;
      }
    }
    ...
    if (rootKey === "client") {
      const cached = await this.cachedOriginsGamedata(context.realm, requestPath);
      if (cached) {
  ...
  private async cachedOriginsGamedata(realm: OriginsRealmDefinition, requestPath: string): Promise<string | null> {
    const normalizedPath = requestPath.replace(/\\/g, "/").toLowerCase();
    const sourceUrl = normalizedPath === ORIGINS_FURNIDATA_FILE
      ? originsRealmGamedataUrl(realm, "furnidata/1")
      : normalizedPath === ORIGINS_PRODUCTDATA_FILE
        ? originsRealmGamedataUrl(realm, "productdata/1")
        : null;
    if (!sourceUrl) return null;
    const cacheDir = join(this.cacheRoot, "gamedata", "realms", realm.id);
...
class EmbeddedRelayController {
  ...
  async start(
    launch: RelayLaunch,
    relayGameConnection: { readonly host: string; readonly port: number },
  ): Promise<void> {
...
function resolveEngineRoot(profileRoot: string): string {
  const override = process.env.SHOCKLESS_ENGINE_ROOT;
  if (override) {
...
function resolveRelayLaunch(profileRoot: string): RelayLaunch {
  ...
  const override = resolveFile(process.env.SHOCKLESS_RELAY);
  if (override) {
    return {
      script: override,
      resourceDir,
...
export function resolveRelayResourceDir(profileRoot: string): string | null {
  const overrideScript = process.env.SHOCKLESS_RELAY;
  const explicitCandidates = [
    process.env.SHOCKLESS_RELAY_RESOURCES,
  ...
  const candidates = [
    ...
    resolve(process.cwd(), "engine", "standalone", "resources", "relay"),
    ...ancestorSiblingCandidates("engine", "standalone", "resources", "relay"),
  ];
  return (
    candidates
      .filter((candidate): candidate is string => Boolean(candidate))
...
function originsRelayGameConnection(
  realm: OriginsRealmDefinition = originsRealmDefinition(DEFAULT_ORIGINS_REALM),
): { readonly host: string; readonly port: number } {
  return {
    host:
      process.env.SHOCKLESS_ORIGINS_TCP_HOST?.trim() ||
      process.env.ORIGINS_TCP_HOST?.trim() ||
      process.env.BRIDGE_TCP_HOST?.trim() ||
      realm.gameHost,
    port:
      positiveInteger(process.env.SHOCKLESS_ORIGINS_TCP_PORT) ??
...
function originsRelayMusConnection(
  gameConnection: { readonly host: string; readonly port: number },
  realm: OriginsRealmDefinition = originsRealmDefinition(DEFAULT_ORIGINS_REALM),
): { readonly host: string; readonly port: number } {
  const gameConnectionOverridden = gameConnection.host !== realm.gameHost || gameConnection.port !== realm.gamePort;
  return {
    host:
      process.env.SHOCKLESS_ORIGINS_MUS_HOST?.trim() ||
      process.env.ORIGINS_MUS_HOST?.trim() ||
      (gameConnectionOverridden ? gameConnection.host : realm.musHost),
    port:
      positiveInteger(process.env.SHOCKLESS_ORIGINS_MUS_PORT) ??
      positiveInteger(process.env.ORIGINS_MUS_PORT) ??
      (gameConnectionOverridden ? Math.min(65535, gameConnection.port + 1) : realm.musPort),
  };
}
...
export function writeShocklessSettings(appDataPath: string, patch: ShocklessSettingsPatch): ShocklessSettings {
  ...
  const next: ShocklessSettings = {
    ...
    entryView:
      patch.entryView === undefined
        ? current.entryView
        : normalizeEntryView(patch.entryView),
    versionCheckBuild:
  ...
  const normalizedNext: ShocklessSettings = {
    ...next,
    customHotelView: next.entryView ? next.customHotelView : true,
  };
  const settingsPath = appDataStorePath(appDataPath, "settings.json");
...
function normalizeEntryView(value: unknown): string | null {
  if (typeof value !== "string") return null;
  const entryView = value.trim();
  return allowedEntryViews.has(entryView) ? entryView : null;
}

export function embeddedResizablePresentation(settingValue: boolean | null, _profileValue: boolean): boolean {
  const override = process.env.SHOCKLESS_RESIZABLE_PRESENTATION;
  if (override === "0" || override?.toLowerCase() === "false") return false;
  if (override === "1" || override?.toLowerCase() === "true") return true;
  if (process.env.SHOCKLESS_FIXED_STAGE === "1") return false;
...
function valueForExternalVariable(lines: readonly string[], key: string): string | null {
  ...
  const line = [...lines].reverse().find((entry) => entry.toLowerCase().startsWith(prefix));
...
function readOptionalText(filePath: string | null): string {
