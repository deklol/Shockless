...
bool processFile(fs::path input, Common::Options &options, bool outputIsDirectory) {
	Common::WriteStats writesBefore = Common::writeStats();
	...
	case Common::kCmdDecompile:
		{
			bool shocklessProfile = options.hasOption("shockless-profile");
			dir->config->unprotect();
			dir->parseScripts();
			if (options.hasOption("dump-scripts")) {
				dir->dumpScripts(castsOutput, !shocklessProfile);
			}
			if (!shocklessProfile) {
			...
			if (shocklessProfile) {
				Common::WriteStats writesAfter = Common::writeStats();
				std::cout << "@shockless-tool-progress {\"source\":\"" << input.filename().string()
					<< "\",\"sourceOutputFiles\":" << (writesAfter.files - writesBefore.files)
					<< ",\"sourceOutputBytes\":" << (writesAfter.bytes - writesBefore.bytes)
					<< ",\"outputFiles\":" << writesAfter.files
					<< ",\"outputBytes\":" << writesAfter.bytes << "}" << std::endl;
			} else {
				Common::log(
...
int main(int argc, char *argv[]) {
	...
	if (fs::is_directory(input)) {
		...
		for (const fs::path &path : sourcePaths) {
